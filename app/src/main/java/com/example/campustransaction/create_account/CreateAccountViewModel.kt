...
class CreateAccountViewModel : ViewModel() {
        ...
        viewModelScope.launch {
            try {
                ...
                if (responseEmailNoExist.Success){
                    ...
                    // 此邮件地址已经创建账户
                    _responseSendEmail.value = responseEmailNoExist
                }
            } catch (e: Exception) {
    ...
    private val _responseSetPassword = MutableLiveData<ResponseBasic>()
    val responseSetPassword: LiveData<ResponseBasic>
        get() = _responseSetPassword

    fun sdkSetPassword(EmailAddress: String, Password: String) {
        viewModelScope.launch {
            try {
                _responseSetPassword.value = MongodbApi.retrofitService.apiSetPassword(EmailAddress, Password)
            } catch (e: Exception) {
                _responseSetPassword.value = ResponseBasic(false,"Failure: ${e.message}")
            }
        }
    }
